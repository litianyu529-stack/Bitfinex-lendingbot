...
- 撤单为 UNKNOWN：进入 SAFE，用两次至少间隔 30 秒的完整 Offers 快照确认该单仍存在或已消失；随后自动恢复此前模式。仍存在时策略可重新撤单，已消失时继续后续计划。
- 钱包转账为 UNKNOWN：进入临时 SAFE。完整钱包快照会显示源钱包剩余余额；两次完整同步后恢复，已完成的全额转账不会因重试而重复转出。
- 挂单提交为 UNKNOWN：每轮同步其请求时间附近的 Offers、Funding Trades 和历史 Offers。唯一匹配时自动绑定；两次权威历史与账户快照均确认不存在时自动关闭；多个候选才保留人工 SAFE。
