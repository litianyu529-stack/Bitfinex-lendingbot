...
1. Worker 停止继续提交，运行状态进入自动恢复中的 `PAUSED`。
...
6. 连续两次权威快照确认后，自动恢复此前的 LIVE；只有人工暂停才不自动启动。
...
- 挂单提交为 UNKNOWN：每轮同步其请求时间附近的 Offers、Funding Trades 和历史 Offers。唯一匹配时自动绑定；两次权威历史与账户快照均确认不存在时自动关闭；多个候选时持续自动对账，但在无法唯一判断前保持只读 PAUSED。
